...
def _maybe_exit_trade(
    ...
    Moderate Progressive SL Trailing:
    - After TP1: SL moves to breakeven (entry)
    - After TP2: SL moves to TP1 level
    - After TP3: SL moves to TP2 level
    - After TP4: SL moves to TP3 level
    """
    ...
    if direction == "bullish":
        hit_tp5 = tp5 is not None and high >= tp5
        hit_tp4 = tp4 is not None and high >= tp4
        ...
        if hit_tp4 and not tp4_hit:
            trade["tp4_hit"] = True
            if tp3 is not None:
                trade["trailing_sl"] = tp3
            if not tp3_hit:
                trade["tp3_hit"] = True
            if not tp2_hit:
                trade["tp2_hit"] = True
        ...
        hit_tp5 = tp5 is not None and low <= tp5
        hit_tp4 = tp4 is not None and low <= tp4
...
def run_backtest(asset: str, period: str) -> Dict:
    ...
    tp2_trail_hits = sum(1 for t in trades if t.get("exit_reason") == "TP2+Trail")
    tp3_trail_hits = sum(1 for t in trades if t.get("exit_reason") == "TP3+Trail")
    tp4_trail_hits = sum(1 for t in trades if t.get("exit_reason") == "TP4+Trail")
    tp5_hits = sum(1 for t in trades if t.get("exit_reason") == "TP5")
    sl_hits = sum(1 for t in trades if t.get("exit_reason") == "SL")

    wins = tp1_trail_hits + tp2_trail_hits + tp3_trail_hits + tp4_trail_hits + tp5_hits
    ...
    notes_text = (
        ...
        f"TP1+Trail ({tp1_trail_hits}), TP2+Trail ({tp2_trail_hits}), TP3+Trail ({tp3_trail_hits}), TP4+Trail ({tp4_trail_hits}), TP5 ({tp5_hits}), SL ({sl_hits})"
    )
    ...
    return {
        ...
        "tp2_trail_hits": tp2_trail_hits,
        "tp3_trail_hits": tp3_trail_hits,
        "tp4_trail_hits": tp4_trail_hits,
        "tp5_hits": tp5_hits,
        "sl_hits": sl_hits,
        "trades": trades,
        "notes": notes_text,
